...
app.post("/criar-pagamento", async (req, res) => {
  try {
    ...
    // Retorna dados do QR Code no formato esperado pelo front-end
    const { qr_code, qr_code_base64 } = pagamento.point_of_interaction.transaction_data;
    res.json({ qr_code, qr_code_base64 });

  } catch (error) {
    console.error(error);
    res.status(500).json({ erro: "Falha ao criar pagamento", detalhes: error.message });
  }
});
